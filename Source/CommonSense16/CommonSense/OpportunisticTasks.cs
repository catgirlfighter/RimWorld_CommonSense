    ...
    class OpportunisticTasks
    {
        ...
        private static Job Cleaning_Opportunity(Job currJob, Pawn pawn, int Limit)
        {
            ...
            if (target != null)
            {
                ...
                {
                    PawnPath pawnPath = target.Map.pathFinder.FindPathNow( source, target, TraverseParms.For(TraverseMode.PassDoors, Danger.None), null, PathEndMode.Touch);
                    if (!pawnPath.Found)
                    {
                    ...
                    pawnPath = building.Map.pathFinder.FindPathNow(source, building, TraverseParms.For(TraverseMode.PassDoors, Danger.None), null, PathEndMode.Touch);
                    if (!pawnPath.Found)
                    {
                        pawnPath.ReleaseToPool();
                        return null;
                    ...
                    pawnPath = target.Map.pathFinder.FindPathNow(building.Position, target, TraverseParms.For(TraverseMode.PassDoors, Danger.None), null, PathEndMode.Touch);
                    if (!pawnPath.Found)
                    {
                        pawnPath.ReleaseToPool();
                        return null;
